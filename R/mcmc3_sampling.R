...
tfr3.mcmc.sampling.extra <- function(mcmc, mcmc.list, countries, posterior.sample,
									iter=NULL, burnin=10000, do.sample.posterior=TRUE, verbose=FALSE, 
									verbose.iter=100) {
	#Run Phase3 mcmc sampling for countries given by the index 'countries' and given the sample of hyperparameters (defined by posterior.sample)
	niter <- iter
	if (is.null(iter))
    	niter <- mcmc$length
	meta <- mcmc$meta
    nr.countries.all <- meta$nr.countries
    nr.countries <- length(countries)
	countries.index <- meta$id_phase3
	
	# get values of the hyperparameters (sample from the posterior)
    hyperparameter.names <- tfr3.parameter.names()
    hyperparameters <- list()
    sampled.index <- if(do.sample.posterior) sample(posterior.sample, niter, replace=TRUE)
    				else posterior.sample[1:niter]
	...
	updated.var.names <- c('mu.c', 'rho.c')
    ...
    for (simu in 1:niter) {
		...
		for(country in countries) {
		 ...
		 if (simu == 1) {
			...
			if (simu + 1 > niter) flush.buffer <- TRUE
		 }
		 #stop('')       
         store.mcmc3(mcmc.orig, append=append, flush.buffer=flush.buffer, countries=countries, verbose=verbose)
	}
	return(mcmc.orig)
}

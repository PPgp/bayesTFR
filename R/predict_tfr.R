...
get.tfr.shift.all <- function(pred, projection.index) {
	...
	if(is.null(pred$traj.shift)) return(shift)
	codes <- meta$regions$country_code
	for(code in names(pred$traj.shift)) {
		idx <- which(code == codes)
		shift[idx] <- pred$traj.shift[[code]][projection.index]
	}
	return(shift)
}

get.tfr.shift <- function(country.code, pred) {
	if(is.null(pred$traj.shift)) return(NULL)
	return(pred$traj.shift[[as.character(country.code)]])
}
...
.bdem.shift <- function(pred, type, country, reset=FALSE, shift=0, from=NULL, to=NULL, verbose = TRUE) {
	meta <- pred$mcmc.set$meta
...
tfr.shift.reset <- function(sim.dir, countries = NULL, ...) {
    if(is.null(countries)) {
	    ...
	    for(country in countries) {
	        pred <- tfr.traj.shift(sim.dir, country, reset=TRUE, ...)
	    }
	invisible(pred)
}
...
tfr.traj.shift <- function(sim.dir, country, reset=FALSE, shift=0, from=NULL, to=NULL, ...) {
	pred <- get.tfr.prediction(sim.dir, ...)
	pred <- .bdem.shift(pred, type='tfr', country=country, reset=reset, 
...
.bdem.stat.set <- function(pred, type, country, values, stat = "median", years=NULL, verbose = TRUE) {
	...
	if(sum(bdem.shift) == 0) bdem.shift <- NULL
	pred$traj.shift[[as.character(country.obj$code)]] <- bdem.shift
	if(verbose) cat('\nThe ', stat, ' of', country.obj$name, 'modified for years', pred.years[which.years], '\n')
	return(pred)
}
